...
(function(){
  ...
  var root = document.getElementById('agon-app'); if (!root) return;
  ...
  document.getElementById('stepper').innerHTML = steps.map(function(st){
    return '<div class="step"><span class="step__dot">'+st.icon+'</span>'+st.label+'</div>';
  }).join('');

  var cta = { start:'Start', crossword:'Submit crossword', question:'Confirm answer', reveal:'Next', coding:'Submit code', review:'See results', results:'Finish' };
  var idx = 0;

  document.getElementById('a-sub').textContent = D.meta.subject || '';
  document.getElementById('a-week').textContent = D.meta.week ? ('Week ' + D.meta.week) : '';

  document.getElementById('explain-toggle').addEventListener('change', function(){ root.classList.toggle('show-explain', this.checked); });
  ...
  function show(name){
    ...
    document.getElementById('cta').textContent = cta[name] || 'Next';
  ...
  document.getElementById('cta').addEventListener('click', function(){ idx = Math.min(idx+1, flow.length-1); show(flow[idx]); });
  document.getElementById('hint').addEventListener('click', function(){ this.disabled = true; alert('Hint: one per attempt.'); });

  function renderCrossword(){
    var host = document.getElementById('cw'); if (host.dataset.done) return;
  ...
  function renderQuestion(){
    var host = document.getElementById('q-host'); var q = (D.questions && D.questions[0]) || null;
  ...
  function clearBlank(blank){ var id = blank.dataset.chip, chip = id && document.getElementById(id); if (chip) chip.classList.remove('is-used'); blank.textContent='____'; blank.classList.remove('is-filled'); blank.removeAttribute('data-chip'); }

  function renderCoding(){
    var seqs = (D.coding && D.coding.sequences) || [];
    document.getElementById('review-explain').textContent = seqs.map(function(s){ return s.explanation || ''; }).join(' ');
    document.getElementById('review-code').innerHTML = seqs.map(function(s){
      var bi = 0, blanks = s.blanks || s.answers || [];
      return esc(s.code || '').replace(/____/g, function(){ var v = blanks[bi++]; return '<b>'+esc(v != null ? v : '____')+'</b>'; });
    }).join('\n\n');

    var host = document.getElementById('coding'); if (host.dataset.done) return;
  ...
  function renderResults(){
    document.getElementById('lb').innerHTML = (D.leaderboard || []).slice().sort(function(a,b){return b.pts-a.pts;})
